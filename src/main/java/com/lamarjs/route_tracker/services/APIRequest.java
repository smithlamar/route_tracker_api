...
import com.lamarjs.route_tracker.models.BusLine.*;abstract

// TODO: swap use of GSON API with implementation using Jackson 
...
public class APIRequest {
	...
	private String key = System.getenv("BTRK");
	...
	public APIRequest(String requestType) throws MalformedURLException {
		// Build the request.
		request = new URL(BUSTIME_REQUEST_BASE + requestType + API_KEY + key + F_JSON);
	}
	...
	public APIRequest(String requestType, String... urlParameters) throws MalformedURLException {
		// Add the ? Parameter designator at the beginning of each parameter.
		// Build the request.
		request = new URL((BUSTIME_REQUEST_BASE + requestType + API_KEY + key + Arrays.toString(urlParameters) + F_JSON)
	 ...
	 * methods. This is because the "routes" element is two levels deep
	...
	public BusLine[] parseRoutes() {
		 ...
		 * Gson, we use a TypeToken to get the correct Type name for an
		 * ArrayList of Type BusRoute.
		 */
